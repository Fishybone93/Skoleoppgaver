...
for (let i = 0; i < words.length; i++) {
    if (words.indexOf(words[i]) !== i && !duplicates.includes(words[i])) {
...
for (let i = 0; i < size; i++) {
    ...
    for (let j = 0; j < size; j++) {
        if (i === 0 || i === size - 1 || j === 0 || j === size - 1 || (i === 3 && j === 3)) {
            row += '*';
        } else {
            row += ' ';
        }
...
console.log("Length of the longest consecutive sequence:", maxStreak);
